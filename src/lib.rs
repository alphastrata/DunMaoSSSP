...
impl PartialOrd for VertexDistance {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        ...
        match self.distance.partial_cmp(&other.distance) {
            Some(Ordering::Equal) => {
                // Break ties by vertex ID for deterministic ordering
                // This ensures the algorithm has consistent behavior
...
/// Adaptive data structure for managing vertex subsets in BMSSP
